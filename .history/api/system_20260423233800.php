...
function queueManualCommand($user_id, $conn, $command_type) {
    ...
    if (!$zones || $zones->num_rows === 0) {
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'No enabled zones found']);
        return;
    }

    $paramsData = ['source' => 'system_control'];
    if ($command_type === 'turn_on' && $duration_minutes > 0) {
        $paramsData['duration_minutes'] = $duration_minutes;
    }
    $params = json_encode($paramsData);

    $stmt = $conn->prepare("INSERT INTO commands (zone_id, command_type, params) VALUES (?, ?, ?)");
    if (!$stmt) {
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Failed to prepare command']);
        return;
    }
    ...
    if ($queued === 0) {
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Failed to queue commands']);
        return;
    }

    echo json_encode([
        'status' => 'success',
        'message' => $command_type === 'turn_on' ? 'Manual start queued' : 'Stop all queued',
