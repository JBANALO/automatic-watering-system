...
function waterZone($user_id, $conn) {
    $modeResult = $conn->query("SELECT auto_mode FROM system_settings WHERE user_id=$user_id LIMIT 1");
    if (!$modeResult || $modeResult->num_rows === 0) {
        http_response_code(409);
        echo json_encode([
            'status' => 'error',
            'message' => 'Manual zone watering is disabled while Auto Mode is ON'
        ]);
        return;
    }

    $modeRow = $modeResult->fetch_assoc();
    if (intval($modeRow['auto_mode'] ?? 1) === 1) {
        http_response_code(409);
        echo json_encode([
            'status' => 'error',
            'message' => 'Manual zone watering is disabled while Auto Mode is ON'
