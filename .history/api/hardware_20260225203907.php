...
if ($method === 'POST' && $action === 'submit') {
    ...
    // Allow testing without zone assignment (zone_id can be NULL for testing)
    if (empty($zone_id) || $zone_id == 0) {
        // For testing: allow data submission but don't update zone
        $zone_id = null;
    }
    ...
    if ($stmt->execute()) {
        // Update zone moisture level (only if zone is assigned)
