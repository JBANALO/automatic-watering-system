...
define('BREVO_SMTP_USER', getenv('BREVO_SMTP_USER') ?: 'a7a4e7001@smtp-brevo.com');
define('BREVO_SMTP_PASSWORD', getenv('BREVO_SMTP_PASSWORD') ?: 'xsmtpsib-6102b571b5276e8375a707e883546335f72ddbc15de603a4f94e9cac7bf9085a-lA5w4R3cVdVSQP4G');
define('BREVO_API_KEY', getenv('BREVO_API_KEY') ?: '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');
...
function handleGoogleLogin($input, $conn) {
    try {
        $googleToken = $input['token'] ?? '';
        $googleData = $input['userData'] ?? [];
        
        if (empty($googleToken) || empty($googleData)) {
            http_response_code(400);
        ...
        $firstName = $conn->real_escape_string($googleData['firstName'] ?? '');
        $lastName = $conn->real_escape_string($googleData['lastName'] ?? '');
        $googleId = $conn->real_escape_string($googleData['id'] ?? '');
        
        if (empty($email) || empty($googleId)) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'Invalid Google data: email or id missing']);
            return;
        }
        
        // Check if user exists by email
        $result = $conn->query("SELECT id, username, email_verified FROM users WHERE email='$email'");
        ...
        if ($result->num_rows > 0) {
            ...
            while (true) {
                $checkResult = $conn->query("SELECT id FROM users WHERE username='$username'");
                if (!$checkResult) {
                    http_response_code(500);
                    echo json_encode(['status' => 'error', 'message' => 'Username check failed: ' . $conn->error]);
                    return;
                }
            ...
            // Google OAuth users don't need password verification
            $tempPassword = password_hash($googleId . time(), PASSWORD_BCRYPT);
            
            $insertQuery = "INSERT INTO users (username, email, password, first_name, last_name, email_verified) 
                           VALUES ('$username', '$email', '$tempPassword', '$firstName', '$lastName', 1)";
            ...
            $zones = ['Front Garden', 'Backyard Lawn', 'Vegetable Garden', 'Side Pathway'];
            foreach ($zones as $zone_name) {
                $zoneQuery = "INSERT INTO zones (user_id, zone_name, moisture_level) VALUES ($userId, '$zone_name', 0)";
                if (!$conn->query($zoneQuery)) {
                    error_log('Zone creation failed: ' . $conn->error);
                }
            }
            
            // Initialize system settings
            $settingsQuery = "INSERT INTO system_settings (user_id) VALUES ($userId)";
            if (!$conn->query($settingsQuery)) {
                error_log('System settings creation failed: ' . $conn->error);
            }
