...
function env_value_or_default($key, $default = null) {
    ...
    return $envValue === '' ? $default : $envValue;
}

// Parse Railway MySQL URL if available
$mysql_url = env_value_or_default('MYSQL_URL');
...
if ($mysql_url) {
    ...
    $host = env_value_or_default('MYSQLHOST', 'localhost');
    $user = env_value_or_default('MYSQLUSER', 'root');
    $pass = env_value_or_default('MYSQLPASSWORD', '');
    $db = env_value_or_default('MYSQLDATABASE', 'irrigation_system');
    $port = env_value_or_default('MYSQLPORT', 3306);
}
